...
if is_manufacturing:
    ...
    # --- 數據輸入區 (手機優化：使用 Tabs 分類) ---
    with st.expander(f"📥 測試參數輸入 ({display_unit})", expanded=True):
        tab_spec, tab_std, tab_acc = st.tabs(["📏 分度與淨重", "📊 重複性 (STD)", "🎯 準確度 (ACC)"])
        ...
        for idx, data in enumerate(range_data):
            ...
            with st.container(border=True):
                ...
                with diag_col1:
                    ...
                    * **關鍵限制**：若 $s < {auto_unit_format(s_threshold_g)}$ ($0.41d$)
                    """)
                    if is_rep_ok:
                ...
                with diag_col2:
                    ...
                    * **允收標準**：誤差 $\le 0.05\\%$
                    * **砝碼限制**：$MPE$ 或 $U$ 需小於 **{mpe_limit_ratio*100:.4f}\\%**
                    """)
                    if is_acc_ok:
                        st.success(f"**【實測對比判斷】**\n\n* 擬用砝碼：`{auto_unit_format(data['acc_w'])}` (✅ 符合規範)")
                    else:
                        st.error(f"**【實測對比判斷】**\n\n* 擬用砝碼：`{auto_unit_format(data['acc_w'])}` (❌ 規格不符)")
                    st.caption(f"💡 砝碼 $U$ 須 $\le {auto_unit_format(mpe_absolute_g)}$")

                st.markdown(f"#### 🛡️ 關鍵秤量能力判定")
                res_c1, res_c2, res_c3, res_c4 = st.columns(4)
                res_c1.metric("最小淨重 (理想)", auto_unit_format(ideal_snw_g))
                res_c2.metric("最小秤重 (實際)", auto_unit_format(actual_min_weight_g))
                res_c3.metric("預期最小淨重", auto_unit_format(data['snw']))
...
* **安全係數 (Safety Factor)**: USP 〈1251〉 建議安全係數應 $\ge 2$ 以確保製程穩定。
""")
