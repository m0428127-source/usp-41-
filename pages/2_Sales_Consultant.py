...
if d2_g:
    ...
    with m_col1: 
        st.metric("d1 理論最小秤重", auto_unit_format(ideal_min_w_d1))
    with m_col2: 
        st.metric("d2 理論最小秤重", auto_unit_format(ideal_min_w_d2))
    with m_col3: 
        # 修正 delta 文字，使其在卡片中顯示美觀
        st.metric("實際最小秤重(判定)", auto_unit_format(usp_min_w), 
                  delta="法規修正(Std < 0.41d)" if is_corrected else None, 
                  delta_color="inverse")
    with m_col4: 
        st.metric("客戶設定 SNW", auto_unit_format(snw_g) if snw_g else "待定")
else:
    m_col1, m_col2, m_col3 = st.columns(3)
    with m_col1: 
        st.metric("機台理論最小秤重", auto_unit_format(ideal_min_w_d1))
    with m_col2: 
        st.metric("實際最小秤重(判定)", auto_unit_format(usp_min_w), 
                  delta="法規修正(Std < 0.41d)" if is_corrected else None, 
                  delta_color="inverse")
